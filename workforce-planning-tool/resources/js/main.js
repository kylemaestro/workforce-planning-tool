...
// Note: These grid options are for the DATA VIEW, which is the
// starting view of any plan. Seperate gridOptions are defined
// for the plan view in plan-view.js
var gridOptions = {
  animateRows: true,
  rowSelection: 'single',
  pagination: true,
...
initDataGrid(gridOptions);

// Initialize the data view grid with column definitions and row data
...
function initPlanGrid() {
  // Grid options and default column properties for the plan view
  gridOptions = {
    animateRows: true,
    rowSelection: 'single',
    pagination: false,
    onGridReady: planViewReady,
...
function dataGridReady() {
  console.log("Data successfully loaded into grid");
  gridOptions.columnApi.autoSizeColumns();
  updatePlanList();
}

function planGridReady() {
  console.log("Plan view successfully created!");
}
...
function removeRow(selectedRow) {
  ...
  gridOptions.api.forEachNode( function(rowNode, index) {
    if(rowNode.data.ID == row.data.ID) {
      console.log("Removing row "+index);
      gridOptions.api.updateRowData({remove: [rowNode.data]});
    }
  });
